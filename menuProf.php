...
if ($result) {
  if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
          ...
          // Récupérer les commentaires liés à la partie avec le nom et le prénom du professeur
          $commentairesSql = "SELECT r.remarque, p.nom AS prof_nom, p.prenom AS prof_prenom
                              FROM remarque AS r
                              INNER JOIN professeur AS p ON r.id_prof = p.id
                              WHERE r.id_partie = '$partieId'";
          $commentairesResult = $db->query($commentairesSql);

          if ($commentairesResult) {
              // Afficher les commentaires existants avec le nom et le prénom du professeur
              echo "<ul>";
              while ($commentaireRow = $commentairesResult->fetch_assoc()) {
                  $commentaire = $commentaireRow['remarque'];
                  $professeurNom = $commentaireRow['prof_nom'];
                  $professeurPrenom = $commentaireRow['prof_prenom'];
                  echo "<li>Professeur: $professeurPrenom $professeurNom, Commentaire: $commentaire</li>";
              }
              echo "</ul>";
...
?>
    ...
    <a href="accueil.php">Quitter l'application</a>
</div>
